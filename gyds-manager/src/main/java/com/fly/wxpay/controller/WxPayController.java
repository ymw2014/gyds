...
public class WxPayController {
	...
	public String getImageUrl(String body, String out_trade_no, String product_id, String total_fee) throws Exception {
		...
		// paramMap.put("product_id", product_id); // 商户根据自己业务传递的参数
		// 当trade_type=NATIVE时必填
		paramMap.put("spbill_create_ip", this.localIp()); // 本机的Ip
		paramMap.put("total_fee", "1"); // 金额必须为整数 单位为分
		paramMap.put("trade_type", "NATIVE"); // 交易类型
		// paramMap.put("sign", WXPayUtil.generateSignature(paramMap,
		// "fenlegou20160226fenlegou20160226"));//根据微信签名规则，生成签名。随机参数可以在商户后台管理系统中进行设置。
		...
		/*
		 * String xmlData = WXPayUtil.mapToXml(paramMap);//把参数转换成XML数据格式 String codeUrl
		 * = getCodeUrl(xmlData); //获取二维码链接
		 */
		return code_url;
	}
	 ...
	 * @return
	 *//*
		 * private String getCodeUrl(String xmlData) { String resXml = null; String
		 * code_url = ""; Map<String, String> map; try {
		 * HttpUtils.postData("https://api.mch.weixin.qq.com/pay/unifiedorder",
		 * xmlData); map = WXPayUtil.xmlToMap(resXml); Object returnCode =
		 * map.get("return_code"); if("SUCCESS".equals(returnCode)) { Object resultCode
		 * = map.get("result_code"); if("SUCCESS".equals(resultCode)) { code_url =
		 * map.get("code_url").toString(); } } } catch (UnrecoverableKeyException e1) {
		 * logger.error(e1.getMessage()); } catch (KeyManagementException e1) {
		 * logger.error(e1.getMessage()); } catch (KeyStoreException e1) {
		 * logger.error(e1.getMessage()); } catch (NoSuchAlgorithmException e1) {
		 * logger.error(e1.getMessage()); } catch (IOException e1) {
